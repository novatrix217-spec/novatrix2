<template><div ref="revealRoot"><PageHero :kicker="t.kicker" :description="t.description"><span v-html="t.heroTitle"/></PageHero>
  ...
  <!-- Équipe : placeholder explicite tant qu'aucune photo réelle n'est fournie (cf. brief — jamais de photo générée/fictive) -->
    ...
    <div class="container-shell">
      ...
      <div class="mx-auto mt-12 max-w-sm">
        <div class="card flex flex-col items-center !p-8 text-center">
          <div class="grid h-28 w-28 place-items-center rounded-full border-2 border-dashed border-[var(--border-strong)] bg-[var(--surface)]" role="img" :aria-label="t.teamPlaceholderAlt">
            <UserRound class="h-10 w-10 text-[var(--text-secondary)]"/>
          </div>
          <p class="mt-5 text-xs font-semibold uppercase tracking-wide text-[var(--text-secondary)]">{{ t.teamPlaceholderLabel }}</p>
          <p class="mt-2 text-sm leading-6 text-[var(--muted)]">{{ t.teamPlaceholderText }}</p>
        </div>
      </div>
    </div>
  </section>
...
import { FileText,Route,TimerReset,UserRound } from 'lucide-vue-next'
...
const t=computed(()=>locale.value==='en'?{
  ...
  teamKicker:'the team', teamTitle1:'Behind the', teamTitle2:'system.', teamPlaceholderLabel:'Photo to be added', teamPlaceholderText:'Team photo and bio coming soon.', teamPlaceholderAlt:'Placeholder — team photo to be added',
}:{
  ...
  teamKicker:'l’équipe', teamTitle1:'Derrière le', teamTitle2:'système.', teamPlaceholderLabel:'Photo à compléter', teamPlaceholderText:'Photo et présentation de l’équipe à venir.', teamPlaceholderAlt:'Emplacement réservé — photo de l’équipe à compléter',
})
