<template><div><PageHero :kicker="t.kicker" :description="t.description">{{ t.title1 }} <span class="text-gradient">{{ t.title2 }}</span><template #actions><button type="button" class="btn-primary" @click="openCalendly">{{ $t('header.bookCall') }} <ArrowRight class="h-4 w-4"/></button></template></PageHero>
...
const localizedItems=computed(()=>{
  ...
  return locale.value==='en'?items.filter(hasCompleteProjectEnglish):items
})
const filters=computed(()=>{
  const cats=[...new Set(localizedItems.value.map(p=>p.category))]
  const catEnMap=new Map(localizedItems.value.map(p=>[p.category,p.categoryEn||p.category]))
  return [{value:'Toutes',label:locale.value==='en'?'All':'Toutes'},...cats.map(c=>({value:c,label:locale.value==='en'?catEnMap.get(c):c}))]
})
const shown=computed(()=>active.value==='Toutes'?localizedItems.value:localizedItems.value.filter(p=>p.category===active.value))
</script>
